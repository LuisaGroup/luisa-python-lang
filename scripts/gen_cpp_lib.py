...
[[nodiscard]] inline bool __hisinf(lc_half x) noexcept { return isinf_impl(lc_float(x)); }
    ...
    for t, native_t in zip(scalar_types, native_types):
        if t == 'half' and is_cpu:
            continue
        print(f"using lc_{t} = {native_t};", file=CPP_LIB_SRC)
    print(file=CPP_LIB_SRC)
...
''', file=CPP_LIB_SRC)
    if is_cpu:
        print(HALF_IMPL, file=CPP_LIB_SRC)
    # vector types
...
return t * t * (lc_{t}(3.f) - lc_{t}(2.f) * t);
    ...
    generate_vector_call("smoothstep", "lc_smoothstep_impl",
                         "hf", ["e0", "e1", "x"])
...
''', file=CPP_LIB_SRC)
    ...
    def gen_element_type(vt, et):
        print(
            f'''template<> struct element_type_<{vt}> {{ using type = {et}; }};''', file=CPP_LIB_SRC)
...
compressed = compress(CPP_LIB_SRC.getvalue())
with open('luisa_lang/codegen/cpp_lib.py', 'w') as f:
    f.write(f'CPP_LIB_COMPRESSED = """{compressed}"""')
